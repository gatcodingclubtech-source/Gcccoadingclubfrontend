...
const CountdownUnit = ({ value, label }) => (
  <div className="flex flex-col items-center">
    <span className="text-xl md:text-3xl font-black text-white leading-none">{value.toString().padStart(2, '0')}</span>
    <span className="text-[7px] md:text-[8px] font-black text-white/40 uppercase tracking-widest mt-1">{label}</span>
  </div>
);
    ...
    <AnimatePresence>
      ...
      >
        <div className="max-w-7xl mx-auto">
          <div className="relative group h-[250px] md:h-[350px] rounded-3xl md:rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10">
            ...
            <div className="relative h-full flex flex-col justify-center p-6 md:p-16">
              <div className="flex flex-col gap-2 md:gap-4 max-w-2xl">
                ...
                <h1 className="text-2xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none drop-shadow-2xl">
                  {banner.title}
                </h1>
                
                <p className="text-[10px] md:text-lg text-white/70 font-medium max-w-lg leading-relaxed mb-2 md:mb-4">
                  {banner.subtitle}
                </p>
                ...
                <div className="flex flex-wrap items-center gap-8 mt-4">
                  <button 
                    ...
                    className={`px-6 md:px-10 py-3.5 md:py-5 rounded-xl md:rounded-2xl bg-white text-black text-[10px] md:text-xs font-black tracking-[0.2em] hover:scale-105 transition-all shadow-2xl flex items-center gap-2 md:gap-3 group/btn`}
                  >
                    JOIN NOW <ArrowRight className="w-4 h-4 md:w-5 md:h-5 group-hover/btn:translate-x-1 transition-transform" />
                  </button>

                  {/* Countdown Timer */}
                  {banner.targetDate && (
                    <div className="flex items-center gap-3 md:gap-6 px-4 md:px-8 py-2 md:py-4 bg-white/5 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-white/10 scale-90 md:scale-100 origin-left">
